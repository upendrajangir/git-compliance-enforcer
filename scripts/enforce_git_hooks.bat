...
icacls "!HOOKS_DIR!" /inheritance:r
icacls "!HOOKS_DIR!" /grant:r *S-1-1-0:(RX)
...
reg add "HKLM\Software\GitForWindows" /v "core.hooksPath" /t REG_SZ /d "!HOOKS_DIR!" /f
if errorlevel 1 (
    echo [ERROR] Failed to set the Git hooks path for all users.
    exit /B 1
)

echo The common Git hooks have been set up successfully in %HOOKS_DIR%.
